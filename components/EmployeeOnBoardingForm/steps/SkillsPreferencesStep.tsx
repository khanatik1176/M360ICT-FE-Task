...
export default function SkillsPreferencesStep({
  ...
  const selectedSkills =
    (watch('skillsPreferences.primarySkills') as Array<{
      skill: string;
      experience: number;
    }>) || [];
  ...
  const addSkill = (skill: string) => {
    ...
    setValue('skillsPreferences.primarySkills', current, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
  ...
  const updateExperience = (skill: string, years: number) => {
    const current = selectedSkills.map((s) =>
      s.skill === skill ? { ...s, experience: years } : s
    );
    setValue('skillsPreferences.primarySkills', current, {
      shouldValidate: true,
      shouldDirty: true,
        ...
        <div className='mt-2 space-y-3'>
          {availableSkills.length === 0 && (
            <div className='text-muted-foreground text-sm'>
              Select a department to see skills
            </div>
          )}
          {availableSkills.map((skill) => {
            const isSelected = selectedSkills.some((s) => s.skill === skill);
            const experience =
              selectedSkills.find((s) => s.skill === skill)?.experience ?? '';
        ...
        <div className='mt-2'>
          <Slider
            ...
            onValueChange={(vals: number[]) =>
              setValue('skillsPreferences.remoteWorkPreference', vals[0], {
                shouldValidate: true,
                shouldDirty: true,
              })
            }
      ...
      {remotePreference > 50 && (
        <div className='relative flex items-center space-x-2'>
          <Checkbox
            ...
            className='text-white'
            onCheckedChange={(checked) =>
              setValue('jobDetails.managerApproval', checked === true, {
                shouldValidate: true,
                shouldDirty: true,
              })
            }
